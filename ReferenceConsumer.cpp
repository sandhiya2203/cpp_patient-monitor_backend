...
const std::string HANDLE_DYNAMIC_BP{"string.ch1.vmd0"};

const std::string HANDLE_DYNAMIC_SPO2{"spo2.ch1.vmd0"};
...
void savePatientData()
{
    ...
    std::ofstream file("/home/sandiya/Downloads/sdclib-master/patient_data.json");
as f:
    return json.load(f)
    ...
    if(!file.is_open())
    {
        std::cout<<"Cannot open patient_data.json"<<std::endl;
        return;
    }
    ...
    std::cout<<"patient_data.json updated"<<std::endl;
}
...
{
    std::string value = p_changedState.getMetricValue().getValue();

    std::cout << "\n===== String Metric Update =====" << std::endl;
    ...
    if(getDescriptorHandle() == HANDLE_DYNAMIC_BP)
    {
        bloodPressure = value;
    }

    if(getDescriptorHandle() == HANDLE_DYNAMIC_SPO2)
    {
        spo2 = value;
    }
...

{
    ...
    std::cout << "Handle : "
<< p_changedState.getDescriptorHandle()
<< std::endl;
...

{
    ...
    std::cout << "        SDC CONSUMER STARTED" << std::endl;
    ...
    std::cout << "\n========== Step 1: WS-Discovery ==========" << std::endl;

    std::cout << "Sending Probe Request..." << std::endl;
    ...
    std::cout << "\n========== Step 2: Provider Connection ==========" << std::endl;

    std::cout << "Connecting to Provider..." << std::endl;  
...

    {
        ...
        DebugOut(DebugOut::Default, "ReferenceConsumer") << t_consumer->getEndpointReference() << std::endl;
            ...
            */
            ...
            t_consumer = std::move(t_availableConsumers.front());
...

    {
...

        {
...

            {
                ...
                std::cout << "Connected to Provider." << std::endl;
                status = "Connected";
...

    {
...

        {
            ...
            std::cout << "\n========== Step 3: Metadata Exchange ==========" << std::endl;

            std::cout << "Requesting Provider Metadata..." << std::endl;

            std::cout << "Provider Metadata Received Successfully." << std::endl;
...
std::make_shared<StringMetricEventHandler>(HANDLE_DYNAMIC_BP);
 
auto spo2_dynamic_get =

std::make_shared<StringMetricEventHandler>(HANDLE_DYNAMIC_SPO2);

 
auto enum_dynamic_get =

std::make_shared<EnumStringEventHandler>(HANDLE_DYNAMIC_GET_ESTRM);
...
&& t_consumer->registerStateEventHandler(bp_dynamic_get.get())
&& t_consumer->registerStateEventHandler(spo2_dynamic_get.get())
...
&& t_consumer->registerStateEventHandler(alert_condition_vmd.get()) && t_consumer->registerStateEventHandler(stream.get()))
          ...
          std::cout << "\n========== Step 5: MDIB Exchange ==========" << std::endl;

          std::cout << "Requesting MDIB..." << std::endl;

            auto numeric_static_get =

    t_consumer->requestState<NumericMetricState>("numeric.ch1.vmd0");
 
            if(numeric_static_get)

            {

                std::cout << "MDIB Received Successfully." << std::endl;

                std::cout << "\nInitial Metric Values" << std::endl;
...

                {
                    ...
                    std::cout << "Heart Rate : "
<< numeric_static_get->getMetricValue().getValue()
<< std::endl;
...
//           auto string_static_get{std::move(t_consumer->requestState<StringMetricState>(HANDLE_DYNAMIC_GET_STRM))};

		auto bp =

    t_consumer->requestState<StringMetricState>(HANDLE_DYNAMIC_BP);
...
<< string_static_get->getMetricValue().getValue()
<< std::endl;
            ...
            //auto enum_static_get{std::move(t_consumer->requestState<EnumStringMetricState>(HANDLE_DYNAMIC_GET_ESTRM))};

		auto spo2 =

    t_consumer->requestState<StringMetricState>(HANDLE_DYNAMIC_SPO2);
...

            {
...

                {
                    ...
                    std::cout << "SPO2 Level: "
<< enum_static_get->getMetricValue().getValue()
<< std::endl;
...

{
    ...
    if (bp->hasMetricValue())

    {

        std::cout << "Blood Pressure: "
<< bp->getMetricValue().getValue()
<< std::endl;
...

{
    ...
    if (spo2->hasMetricValue())

    {

        std::cout << "SPO2 Level: "
<< spo2->getMetricValue().getValue()
<< std::endl;
...
<< "Commit result metric state: " << fis.waitReceived(InvocationState::Fin, 2000) << "\n";
            ...
            // Keep consumer alive to receive metric reports

            std::cout << "Waiting for metric updates..." << std::endl;
...
t_consumer->unregisterStateEventHandler(bp_dynamic_get.get());

t_consumer->unregisterStateEventHandler(spo2_dynamic_get.get());
